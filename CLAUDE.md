...
- Pages with `bare: true` frontmatter drop the site chrome (side nav + webring); build.py strips them per-page. Fishbowl uses this and goes dark (`body.page-fishbowl`) with its own `← xavierrg.com` home link; its design language is serif sentences, not small-caps labels.
